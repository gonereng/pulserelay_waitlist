...
export function WaitlistForm({ variant = "hero" }: WaitlistFormProps) {
  ...
  useEffect(() => {
    ...
    if (verified === "success") {
      ...
      setStatus("success");
      setMessage("This email is already verified and on the waiting list.");
      window.history.replaceState({}, "", "/");
      ...
      <div className="max-w-md mx-auto text-center">
        <div className="bg-primary-muted border border-primary text-primary p-4 rounded">
          <span className="material-symbols-outlined text-2xl align-middle mr-2">
            {message.includes("already") ? "info" : "mark_email_unread"}
          </span>
          {message}
        </div>
      </div>
    ...
    >
      ...
      <label className="flex items-start gap-2 cursor-pointer select-none">
        ...
        <span className="text-sm text-on-surface-variant">
          I agree that my email is stored and that I am contacted about news
          regarding PulseRelay.
        </span>
      </label>
      {status === "error" && (
